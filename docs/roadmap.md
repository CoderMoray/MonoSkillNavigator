...
- [ ] README 参考 [awesome-readme](https://github.com/matiassingers/awesome-readme) 模板，规范 Skill 包内文档结构
...
- [ ] 发布策略：`rejected` 版本阻断同 semver 再发布；admin override
- [ ] 发布新版本自动重新上架（下架 Skill 的恢复策略已部分实现，需文档化）

### 1.4 详情页与社区操作

- [x] Issue / Rating / Contributor API 与详情页 modal 入口
- [ ] 强化登录门禁：**所有会修改状态的操作**均需登录（发布、评分、Issue、Contributor、下架、删除等统一策略）
...
- [ ] 大页面拆分（详情、发布、广场 → `components/skills/*`）
...
2. **HaluCatch 质量检查**（功能性 / 幻觉类）— 异步 Worker
...
- [ ] 安装 / 调用 ClawHub CLI，对接 HaluCatch（AI 搜、任务集评估）
- [ ] `packages/evaluator` 的 `halucatch-adapter` 从占位实现为真实调用
...
| 项 | 说明 |
| --- | --- |
| AI Skill 目录管理 | Agent 如何在本地发现、更新、切换 Skill；与平台 `install` 路径是否统一 |
| Summary 覆盖 description | Web 发布会用表单 Summary 覆盖 manifest `description`，可能触发「缺少触发场景」类 finding |
| 审查快照非实时 | 详情页展示发布时入库的报告，改本地代码不会更新已发布版本 |
| PostgreSQL 脚本缺失 | 缺少独立 migration/seed 脚本，环境复现依赖 `storage` 内嵌初始化 |
| 综合分语义 | 多维度分数合成方式未统一，需先定义再决定是否废弃 |

---

## 优先级建议

1. **短期**：登录门禁统一、文件夹上传、发布进度、tests/CI、前端页面拆分
2. **中期**：Worker 队列、HaluCatch、雷达图、搜索增强、creator profile
3. **长期**：MCP 写入、VirusTotal/Skill Vetter、admin override、Keynote 视觉全量落地
