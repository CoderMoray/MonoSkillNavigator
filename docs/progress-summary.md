...
- 发布后会自动执行静态审查和 HaluCatch 五维可靠性评估。

### 3. 审查与评分

- 已实现质量、安全和可靠性三个独立评分维度，不计算综合分；质量汇总平台合规与质量规则，安全采用 SkillSpector 静态扫描。
- 已接入 HaluCatch，对地基、代码风险、规则、护栏和复杂度做静态可靠性检查；`tests/*.json` 任务集评估作为无 Python 运行时的回退。
...
- HaluCatch 当前是静态可靠性评估，不会执行 Skill 脚本；需要实际运行 Agent 的动态评估仍需隔离执行环境。
